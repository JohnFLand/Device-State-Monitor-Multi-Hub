...
Device names are clickable links to their hub's device edit page. In-row action buttons let you toggle devices ON or OFF without leaving the page.
...
| **Toggle Command & Health Monitor Settings** | Toggle to reveal Maker API credentials for Hub #1. These are used for two purposes: (1) in-row ON/OFF toggle buttons in the report tables; (2) the Load / Select All / Clear All actions in the health device picker. If left blank, toggle buttons are suppressed and health selection reverts to a manual capability picker. |
...
Hubitat's Maker API does not expose disabled state reliably, so disabled devices cannot be filtered automatically on remote hubs. If a disabled device appears in the Health/Activity table, the recommended remedies in order of preference are:
...
Toggle buttons in the Action column send an **off** command via the Maker API. The row updates in-place without a full page refresh.

### OFF Devices Table

Lists every monitored device currently reporting switch state **off**. Same dual-monitoring highlight applies.

Toggle buttons send an **on** command.

### Unknown State Table

Lists monitored devices reporting a switch state other than on or off (e.g. null, initializing, or an unrecognized value). Can be hidden in Sort & Display Options. Toggle buttons offer both **→ ON** and **→ OFF** commands.
...
## In-Row Toggle Buttons

Toggle buttons appear in the Action column of the ON, OFF, and Unknown tables when Maker API credentials are configured for the relevant hub. They work as follows:

- The command is sent via the Maker API using `fetch()` — the page does not reload.
- The button shows **…** while the command is in flight.
- On success, the row's State cell and button label update immediately (optimistic update).
- On failure (network error or non-200 HTTP status), the button shows a brief error indicator and reverts.

Toggle buttons do not appear in the Health/Activity table — that table is for monitoring only.
...
4. To enable toggle buttons and health Select All/Clear All on Hub #1: toggle **Show / Edit Toggle Command & Health Monitor Settings**, enter Maker API credentials for Hub #1, then use the **Hub #1 Health Device List Actions** dropdown to Load and Select All.
